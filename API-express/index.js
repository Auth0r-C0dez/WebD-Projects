...
app.get("/jokes/:id",(req,res) => {
  ...
  const jokez = jokes.find((joke) => joke.id ===id);
  res.json(jokez);
})
//3. GET a jokes by filtering on the joke type
app.get("/filter",(req,res) => {
  const requiremnt = req.query.type;
  const filtdJOke = jokes.filter((joke) => joke.jokeType === requiremnt);
  res.json(filtdJOke);
})
//4. POST a new joke
...
app.put("/jokes/:id",(req,res) => {
  ...
  const exact = jokes.findIndex((joke) => joke.id=== Id);
  jokes[exact] =details;
...
app.patch("/jokes/:id",(req,res) => {
  ...
  const replacing = jokes.findIndex((joke) => joke.id ===id);
  jokes[replacing] = rep;
...
app.delete("/jokes/:id",(req,res) =>{
  const id = parseInt(req.params.id);
  const findn = jokes.findIndex((joke) => joke.id === id);
  if(findn>-1) {
    jokes.splice(findn,1);
    res.sendStatus(200);
  } else {
    res
    .status(404)
